...
from experim.ordclass.load_dataset_dl_classif import load_utk
from experim.ordclass.config_experim_dl_classif import EXPERIMENTS  # import experimental scenarios from an auxiliary file


# path and files
PATH_RESULTS = 'results/ordclass'
FILE_REFERENCE = 'DL Classif-UTK Task {0}_Experim {1:02d}_Scoring {2}'
FILE_SUMMARY = FILE_REFERENCE + '_summary.json'
FILE_Y_TRUE_TEST = FILE_REFERENCE + '_y-true-test.npy'
FILE_Y_PRED_TEST = FILE_REFERENCE + '_y-pred-test.npy'
FILE_PROBA_PRED_TEST = FILE_REFERENCE + '_proba-pred-test.npy'
...
TEMPLATE_Y_TR_VALID = 'y-true-valid_trial-{0}_{1:%Y-%m-%d_%H-%M-%S,%f}.npy'
TEMPLATE_Y_PR_VALID = 'y-pred-valid_trial-{0}_{1:%Y-%m-%d_%H-%M-%S,%f}.npy'
TEMPLATE_P_PR_VALID = 'proba-pred-valid_trial-{0}_{1:%Y-%m-%d_%H-%M-%S,%f}.npy'
TEMPLATE_Y_TR_TEST = 'y-true-test_trial-{0}_{1:%Y-%m-%d_%H-%M-%S,%f}.npy'
TEMPLATE_Y_PR_TEST = 'y-pred-test_trial-{0}_{1:%Y-%m-%d_%H-%M-%S,%f}.npy'
TEMPLATE_P_PR_TEST = 'proba-pred-test_trial-{0}_{1:%Y-%m-%d_%H-%M-%S,%f}.npy'
...
if __name__ == '__main__':
    ...
    hyparams_fixed['input_height'] = IMG_HEIGHT_B0
    hyparams_fixed['input_width'] = IMG_WIDTH_B0
    hyparams_fixed['input_channels'] = IMG_CHANNELS_RGB

    hyparams_fixed['epochs'] = N_EPOCHS_UTK

    # load data
    data_info = load_utk(task=task_utk)
    ...
    counts_train, priors_train = data_info['counts_train'], data_info['priors_train']

    # pre-initialize the classification model
    model = OrdinalCNN2DEffB0(n_classes=n_classes)

    # set class counts and class priors
    hyparams_fixed['class_counts'] = counts_train
    hyparams_fixed['class_priors'] = priors_train
    ...
    hyparams_fixed['batch_size'] = BATCH_SIZE_UTK

    X_train = (X_train
               .cache()
               .shuffle(n_train, reshuffle_each_iteration=True)
               .batch(BATCH_SIZE_UTK)
    ...
    def optim_objective(trial):
        ...
        y_true_valid = np.zeros(shape=(n_valid,), dtype=np.uint8)
        y_pred_valid = np.zeros(shape=(n_valid,), dtype=np.uint8)
        # if out_type != 'regression':
        #     proba_pred_valid = np.zeros(shape=(n_valid, n_classes), dtype=np.float32)
        ...
        for X_valid_, y_true_valid_ in X_valid:
            ...
            y_true_valid_ = y_true_valid_.numpy().flatten()
            y_true_valid[count_valid:(count_valid + batch_valid_)] = y_true_valid_

            y_pred_valid_ = model_._predict(X_valid_)
            y_pred_valid_ = y_pred_valid_.numpy().flatten()
            y_pred_valid[count_valid:(count_valid + batch_valid_)] = y_pred_valid_

            # if out_type != 'regression':
            #     proba_pred_valid_ = model_._predict_proba(X_valid_)
            #     proba_pred_valid_ = proba_pred_valid_.numpy()
            #     proba_pred_valid[count_valid:(count_valid + batch_valid_), :] = proba_pred_valid_
        ...
        for X_test_, y_true_test_ in X_test:
            ...
            y_true_test[count_test:(count_test + batch_test_)] = y_true_test_

            y_pred_test_ = model_._predict(X_test_)
            y_pred_test_ = y_pred_test_.numpy().flatten()
            y_pred_test[count_test:(count_test + batch_test_)] = y_pred_test_

            if out_type != 'regression':
                proba_pred_test_ = model_._predict_proba(X_test_)
                proba_pred_test_ = proba_pred_test_.numpy()
                proba_pred_test[count_test:(count_test + batch_test_), :] = proba_pred_test_

            count_test += batch_test_

        # save test data to disk (npy)
        ...
        np.save(os.path.join(PATH_OPTUNA_TEMP_FILES, file_y_true_test), y_true_test)
        np.save(os.path.join(PATH_OPTUNA_TEMP_FILES, file_y_pred_test), y_pred_test)
        if out_type != 'regression':
            np.save(os.path.join(PATH_OPTUNA_TEMP_FILES, file_proba_pred_test), proba_pred_test)
    ...
    optim_study = create_study(sampler=TPESampler(n_startup_trials=N_STARTUP_UTK),
                               direction=direction)

    # optimize and measure the time involved
    time_start_optim = time.process_time()

    optim_study.optimize(optim_objective, n_trials=N_TRIALS_UTK, gc_after_trial=True)
    ...
    file_y_true_test = FILE_Y_TRUE_TEST.format(task_utk, idx_exper, scoring)
    file_y_pred_test = FILE_Y_PRED_TEST.format(task_utk, idx_exper, scoring)
    if out_type != 'regression':
        file_proba_pred_test = FILE_PROBA_PRED_TEST.format(task_utk, idx_exper, scoring)
    ...
    y_true_test = np.load(os.path.join(PATH_RESULTS, file_y_true_test))
    y_pred_test = np.load(os.path.join(PATH_RESULTS, file_y_pred_test))
    # if out_type != 'regression':
    #     proba_pred_test = np.load(os.path.join(PATH_RESULTS, file_proba_pred_test))
    ...
    dict_summary = {'dataset': data_name,
                    ...
                    'n_trials': N_TRIALS_UTK, 'n_startup': N_STARTUP_UTK,
                    'trials_info': trials_info,
                    'time_optim': time_optim,
                    'score_optim_best': score_optim_best,
                    'score_test': score_test}

    file_summary = FILE_SUMMARY.format(task_utk, idx_exper, scoring)
