...
from experim.ordclass.load_segment_bach import load
...
if __name__ == '__main__':
    ...
    hyparams_ = HYPARAMS.HYPARAMS_SEGMENT.copy()
    hyparams.update(hyparams_)

    hyparams_ = HYPARAMS.HYPARAMS_SEGMENT_2D.copy()
    hyparams.update(hyparams_)
    ...
    y_shape = list(dataset[CONFIG.TAG_SHAPE_Y])
    proba_shape = y_shape + [n_classes]

    y_test_shape = tuple([n_test] + y_shape)
    proba_test_shape = tuple([n_test] + proba_shape)

    y_true_test = np.zeros(shape=y_test_shape, dtype=np.uint8)
    y_pred_test = np.zeros(shape=y_test_shape, dtype=np.uint8)
    if out_type == 'regress':
        proba_pred_test = None
    else:
        proba_pred_test = np.zeros(shape=proba_test_shape, dtype=np.float32)
    ...
    for X_test_, y_true_test_ in X_test:
        ...
        y_true_test_ = tf.squeeze(y_true_test_).numpy()
        y_true_test[cum_test:(cum_test + batch_test_), ...] = y_true_test_

        y_pred_test_ = model._predict(X_test_)
        y_pred_test_ = tf.squeeze(y_pred_test_).numpy()
        y_pred_test[cum_test:(cum_test + batch_test_), ...] = y_pred_test_

        if out_type != 'regress':
            proba_pred_test_ = model._predict_proba(X_test_)
            proba_pred_test_ = tf.squeeze(proba_pred_test_).numpy()
            proba_pred_test[cum_test:(cum_test + batch_test_), ...] = proba_pred_test_

        cum_test += batch_test_
    ...
    results = {
        ...
        'backbone_model': 'UNet(2D)|EfficientNetB5',
        'hyperparameters': hyparams,
        'n_epochs_max': n_epochs_max, 'n_epochs_run': n_epochs_run, 'n_epochs_best': n_epochs_best,
