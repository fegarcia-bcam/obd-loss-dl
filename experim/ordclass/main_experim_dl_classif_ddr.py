...
from src.ordclass.OrdinalCNN import OrdinalCNN2DEffB5

from experim.ordclass.load_dataset_dl_classif import load_ddr
...
FILE_REFERENCE = 'DL Classif-DDR_Experim {0:02d}_Scoring {1}'
...
if __name__ == '__main__':
    ...
    hyparams_fixed['epochs'] = N_EPOCHS_DDR
    ...
    model = OrdinalCNN2DEffB5(n_classes=n_classes)
    ...
    hyparams_fixed['batch_size'] = BATCH_SIZE_DDR

    X_train = (X_train
               .cache()
               .shuffle(n_train, reshuffle_each_iteration=True)
               .batch(BATCH_SIZE_DDR)
    ...
    optim_study = create_study(sampler=TPESampler(n_startup_trials=N_STARTUP_DDR),
                               direction=direction)

    # optimize and measure the time involved
    time_start_optim = time.process_time()

    optim_study.optimize(optim_objective, n_trials=N_TRIALS_DDR, gc_after_trial=True)
    ...
    dict_summary = {'dataset': data_name,
                    ...
                    'n_trials': N_TRIALS_DDR, 'n_startup': N_STARTUP_DDR,
