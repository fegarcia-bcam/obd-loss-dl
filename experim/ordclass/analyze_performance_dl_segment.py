...
TAG_DL_SEGMENT = 'DL Segment'

TAG_INFO = '_summary.json'
TAG_Y_TRUE = '_y-true-test.memmap'
TAG_Y_PRED = '_y-pred-test.memmap'
TAG_PROBA_PRED = '_proba-pred-test.memmap'
...
def analyze(path, file_info):
    ...
    n_samples = info['n_test']
    y_shape = info['y_shape']
    proba_shape = info['proba_shape']

    y_shape = tuple([n_samples] + y_shape)
    proba_shape = tuple([n_samples] + proba_shape)

    # load ground truth and predictions
    y_true = np.memmap(os.path.join(path, file_y_true),
                       shape=y_shape, mode='r', dtype=np.uint8)
    y_pred = np.memmap(os.path.join(path, file_y_pred),
                       shape=y_shape, mode='r', dtype=np.uint8)
    has_proba = os.path.exists(os.path.join(path, file_proba_pred))
    if has_proba:
        proba_pred = np.memmap(os.path.join(path, file_proba_pred),
                               shape=proba_shape, mode='r', dtype=np.float32)
...
if __name__ == '__main__':
    ...
    for file_ in os.listdir(PATH_RESULTS):
        if file_.startswith(TAG_DL_SEGMENT) and file_.endswith(TAG_INFO):
            l_files_info.append(file_)
    l_files_info = sorted(l_files_info)
    n_files_info = len(l_files_info)

    l_analysis = []
    for file_info_ in tqdm(l_files_info, total=n_files_info, desc='Analyzing performance - DL Segment'):
    ...
    df_analysis.to_csv(os.path.join(PATH_RESULTS, TAG_DL_SEGMENT + EXT_ANALYSIS), index=False)
